...
public class RenderObjectVbo {
    ...
    public void upload(MeshData meshData, boolean shouldResort) throws RuntimeException {
        ...
        if (RenderSystem.isOnRenderThread() && meshData != null) {
            ...
            if (this.vertexBuffer == null) {
                this.vertexBuffer = RenderSystem.getDevice().createBuffer(() -> this.name.get() + " VertexBuffer", 40,
                        expectedSize);
            ...
            if (shouldResort && meshData.indexBuffer() != null) {
                if (this.indexBuffer != null && this.indexBuffer.size() >= meshData.indexBuffer().remaining()) {
                    ...
                    this.indexBuffer = RenderSystem.getDevice().createBuffer(() -> this.name.get() + " IndexBuffer", 72,
    ...
    protected VertexSorting createVertexSorter(Vec3 pos, BlockPos origin) {
        return VertexSorting.byDistance((float) (pos.x - (double) origin.getX()),
                (float) (pos.y - (double) origin.getY()), (float) (pos.z - (double) origin.getZ()));
    }
    ...
    protected void bindTexture(Identifier id, int textureId, int width, int height) throws RuntimeException {
        ...
        try {
            // Verify that we potentially have the correct texture by checking various
            // values
    ...
    protected void draw(MeshData meshData, boolean shouldResort, boolean setColor, boolean useOffset)
        ...
        if (RenderSystem.isOnRenderThread()) {
            if (meshData == null) {
                ...
                if (this.indexCount < 1) {
                    // MiniHUD.LOGGER.warn("RenderContext#draw() [{}] --> upload()",
                    // this.name.get());
                    this.upload(meshData, shouldResort);
                }
            }
            ...
            if (this.indexCount > 0) {
                float[] rgba = new float[] { ARGB.redFloat(this.color), ARGB.greenFloat(this.color),
                        ARGB.blueFloat(this.color), ARGB.alphaFloat(this.color) };
    ...
    protected void drawPost(@Nullable RenderTarget otherFb, boolean setColor, boolean useOffset)
            throws RuntimeException {
        this.ensureSafeNoTexture();
        ...
        if (this.indexCount > 0) {
            ...
            // MiniHUD.LOGGER.warn("RenderContext#drawPost() [{}] --> drawInternal()",
            // this.name.get());
    ...
    private void drawInternal(@Nullable RenderTarget otherFb, float[] rgba, boolean setColor, boolean useOffset)
            throws RuntimeException {
        this.ensureSafeNoTexture();
        ...
        if (RenderSystem.isOnRenderThread()) {
            ...
            // MiniHUD.LOGGER.warn("RenderContext#drawInternal() [{}] --> new renderPass",
            // this.name.get());
            ...
            try (RenderPass pass = device.createCommandEncoder()
                ...
                // MiniHUD.LOGGER.warn("RenderContext#drawInternal() [{}] renderPass -->
                // setPipeline() [{}]", this.name.get(), this.shader.getLocation().toString());
                ...
                // MiniHUD.LOGGER.warn("RenderContext#drawInternal() [{}] renderPass -->
                // setVertexBuffer() [0]", this.name.get());
                ...
                // MiniHUD.LOGGER.warn("RenderContext#drawInternal() [{}] renderPass -->
                // drawIndexed() [0, {}]", this.name.get(), this.bufferIndex);
                pass.drawIndexed(0, 0, this.indexCount, 1);
            }

            // MiniHUD.LOGGER.warn("RenderContext#drawInternal() [{}] --> END",
            // this.name.get());
        }
    }
