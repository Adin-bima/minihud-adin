...
public class OverlayRendererBlockGrid extends OverlayRendererBase {
    ...
    protected void allocateBuffers() {
        this.clearBuffers();
        this.renderObjects.add(new RenderObjectVbo(() -> this.getName() + " Lines",
    ...
    private void renderOutlines(Vec3 cameraPos, Minecraft mc, ProfilerFiller profiler) {
        ...
        BufferBuilder builder = ctx.start(() -> "minihud:block_grid/outlines",
                MaLiLibPipelines.DEBUG_LINES_MASA_SIMPLE_LEQUAL_DEPTH);
        ...
        switch (mode) {
            case ALL:
                ...
                break;
            case NON_AIR:
                this.renderLinesNonAir(cameraPos, this.cameraEntity.level(), this.lastUpdatePos, radius, color,
                        this.glLineWidth, builder);
                break;
            case ADJACENT:
                this.renderLinesAdjacentToNonAir(cameraPos, this.cameraEntity.level(), this.lastUpdatePos, radius,
            ...
            for (int z = startZ; z <= endZ; ++z) {
                ...
                for (int y = startY; y <= endY; ++y) {
                    ...
                    if (!chunk.getBlockState(posMutable).isAir()) {
                        fi.dy.masa.malilib.render.RenderUtils.drawBlockBoundingBoxOutlinesBatchedLines(posMutable,
                                cameraPos, color, 0.001, lineWidth, buffer);
                    }
                }
            }
        }
                    ...
                    if (chunk.getBlockState(posMutable).isAir()) {
                        for (Direction side : PositionUtils.VERTICAL_DIRECTIONS) {
                            ...
                            if (!chunk.getBlockState(posMutable2).isAir()) {
                                fi.dy.masa.malilib.render.RenderUtils.drawBlockBoundingBoxOutlinesBatchedLines(
                                        posMutable, cameraPos, color, 0.001, lineWidth, buffer);
                                break;
                            }
                        }
