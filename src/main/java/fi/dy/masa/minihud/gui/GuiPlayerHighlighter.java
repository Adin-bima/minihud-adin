...
public class GuiPlayerHighlighter extends GuiBase {
    ...
    private void refreshColorEditor() {
        int r = 100, g = 200, b = 255, a = 255;
        if (selectedEntryIndex >= 0 && selectedEntryIndex < PlayerHighlighterConfigs.getEntries().size()) {
            PlayerEntry e = PlayerHighlighterConfigs.getEntries().get(selectedEntryIndex);
            r = e.getColorR();
            g = e.getColorG();
            b = e.getColorB();
            a = e.getColorA();
    ...
    public void initGui() {
        ...
        for (int i = 0; i < entries.size(); i++) {
            ...
            rowX += 53;
            GuiTextFieldGeneric nameField = new GuiTextFieldGeneric(rowX, y, 160, 17, Minecraft.getInstance().font);
